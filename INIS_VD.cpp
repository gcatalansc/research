...
vo INIS_VD::Initialize(in _x_length)
{
	//---------------------------------------------------------------------------------
...
vo INIS_VD::Create(dou *_input, dou _input_evaluation, in _rs_srg, dou _al_srg)
{
	//---------------------------------------------------------------------------------
...
vo INIS_VD::CreateRelation(in _rl_type, in _rl_id, in _upci_type, in _upci_id)
{
	//--------------------------------------------------------------------------------
...
vo INIS_VD::DeleteRelation(in _index)
{
	//---------------------------------------------------------------------------------
...
dou INIS_VD::Differentiate(dou *_in_rg)
{
	//---------------------------------------------------------------------------------
	//---------------------------------------------------------------------------------
	...
	for (in i1 = 0; i1 < this->X_LENGTH; i1++)
	{
		...
		if (abs(_in_rg[i1]) > abs(this->IN_RG[i1]))
		{
			//-------------------------------------------------------------------------
			//diferenciación de estructuras para obtener congruencia porcentual acumulada.
			congruence += 100.0 - ((abs(_in_rg[i1] - this->IN_RG[i1]) / abs(_in_rg[i1])) * 100.0);
		}
		...
		else
		{
			//-------------------------------------------------------------------------
			//diferenciación de estructuras para obtener congruencia porcentual acumulada.
			congruence += 100.0 - ((abs(_in_rg[i1] - this->IN_RG[i1]) / abs(this->IN_RG[i1])) * 100.0);
		}
	}
...
in INIS_VD::GetRelationIndex(in _rl_type, in _rl_id, in _upci_type, in _upci_id)
{
	//---------------------------------------------------------------------------------
...
in INIS_VD::GetNextDifferentialInputRelation(in _rl_start_index)
{
	//---------------------------------------------------------------------------------
	//obtención de la cantidad de relaciones.
	in rl_amount = this->GetRelationAmount();
	...
	for (in i1 = _rl_start_index + 1; i1 < rl_amount; i1++)
	{
		...
		if (this->RL.at(i1).at(0) == -INDFRLIS_VD_TYPE)
		{
			//-------------------------------------------------------------------------
...
in INIS_VD::GetNextOutputRelationS(in _rl_start_index)
{
	//---------------------------------------------------------------------------------
	//obtención de la cantidad de relaciones.
	in rl_amount = this->GetRelationAmount();
...
in INIS_VD::GetPreviousInputRelationS(in _rl_start_index)
{
	//---------------------------------------------------------------------------------
...
in INIS_VD::GetPreviousDifferentialInputRelation(in _rl_start_index)
{
	//---------------------------------------------------------------------------------
	//bucle para obtener el índice de la anterior relación INDFRLIS entrante.
	for (in i1 = _rl_start_index - 1; i1 >= 0; i1--)
...
in INIS_VD::GetLastInputDifferentialRelationIndex()
{
	//---------------------------------------------------------------------------------
	//obtención de la cantidad de relaciones.
	in rl_amount = this->GetRelationAmount();
...
vo INIS_VD::Set(dou *_input, dou _input_evaluation, in _rs_srg, in _cs_srg, dou _al_srg, dedein &_rl)
{
	//---------------------------------------------------------------------------------
	//paralelización del bucle for.
	#pragma omp parallel for
